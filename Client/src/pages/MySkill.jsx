...
//   const [skills, setSkills] = useState([]);
//   const [loading, setLoading] = useState(true);
// const [showSkillQuery, setShowSkillQuery] = useState(false);
...
//   // ensure pdfText is always in sync with the editingSkill (when editing)
//   useEffect(() => {
//     let cancelled = false;
...
//       if (!editingSkill || !editingSkill._id) {
//         setPdfText("");
//         // also reset template-related editing values
//         setFormData((p) => ({
//           ...p,
//           TemplateType: "",
//           TemplateImage: null,
...
//         // heuristic to detect template type (keeps edit UI helpful)
//         let detected = "main";
//         if (text.includes("-") && text.includes(",")) detected = "sub";
//         if (text.includes("✓")) detected = "main";
//         if (!text.includes(",") && !text.includes("-") && text.trim().length < 80) detected = "main";

//         setFormData((p) => ({
//           ...p,
//           TemplateType: p.TemplateType || detected,
//           TemplateData: p.TemplateData || text,
//         }));
//       } catch (err) {
//         if (!cancelled) {
//           console.warn("Could not load pdfText for editingSkill:", err?.message || err);
...
//       setPdfText(text);
//       // detect template type based on existing text (heuristic)
//       let detectedTemplate = "main";
//       if (text.includes("-") && text.includes(",")) detectedTemplate = "sub";
//       if (text.includes("✓")) detectedTemplate = "main";
//       if (!text.includes(",") && !text.includes("-")) detectedTemplate = "main";
...
//   // inside handleSubmit, replace the FormData construction with this:
// try {
//   const fd = new FormData();
//   fd.append("UserId", userId);
//   if (formData.SkillId) fd.append("SkillId", formData.SkillId);
...
//   // TEMPLATE fields
//   if (formData.TemplateType) fd.append("TemplateType", formData.TemplateType);
//   if (formData.TemplateImage) fd.append("TemplateImage", formData.TemplateImage);

//   // ALWAYS send EditedText so backend can regenerate (for both add & edit)
//   // Prefer current live preview text (pdfText) then TemplateData
//   const edited = (pdfText && pdfText.trim()) ? pdfText : (formData.TemplateData || "");
//   if (edited) fd.append("EditedText", edited);
...
//   /* -------------------- small handlers for files & template fields -------------------- */
//   const onCertificateChange = (file) => {
//     setFormData((p) => ({ ...p, Certificate: file }));
//   };
//   const onContentFileChange = (file) => {
...
//     setFormData((p) => ({ ...p, TemplateType: type, TemplateData: "", TemplateImage: null }));
//     // keep pdfText in sync when switching types
//     setPdfText("");
//   };

//   const onTemplateImageChange = (file) => {
//     setFormData((p) => ({ ...p, TemplateImage: file }));
//   };

//   const onTemplateDataChange = (val) => {
//     setFormData((p) => ({ ...p, TemplateData: val }));
//     setPdfText(val); // sync preview / edited text
//   };
...
//     <button
//       onClick={() => setShowSkillQuery((s) => !s)}
//       className={`inline-flex items-center gap-2 px-3 py-2 rounded-md border shadow-sm transition ${
//         showSkillQuery ? "bg-[#B87C4C] text-white" : "bg-white hover:bg-[#F7F4EA]"
//       }`}
//       title="Open Skill Query"
//     >
//       <FiUpload /> Skill Query
//     </button>
//   </div>
//       {successMsg && (
//         <div className="mb-4 px-4 py-2 rounded-md bg-[#A8BBA3]/20 text-[#31513A] border border-[#A8BBA3]/60 text-sm">
...
//                               View Topic pdf
//                             </a>
//                             {/* optional: show a button to replace if user really wants to update */}
//                             {/* <button onClick={() => setFormData(p => ({...p, ContentFile: null}))} className="ml-3 text-sm">Replace</button> */}
//                           </p>
//                         ) : (
//                           <>
//                             <input
...
//                               className="w-full border border-[#CBBFAE] px-3 py-2 rounded-md focus:ring-2 focus:ring-[#B87C4C]/30 bg-white/80"
//                             />
//                             {/* show message when no current file */}
//                             {!editingSkill && !formData.ContentFile && (
//                               <p className="text-xs text-gray-500 mt-1">You can upload a topics PDF (optional).</p>
//                             )}
//                           </>
//                         )}
//                       </div>
...
//                               type="file"
//                               accept="image/*"
//                               onChange={(e) => onTemplateImageChange(e.target.files[0] || null)}
//                               className="w-full border border-[#CBBFAE] px-3 py-2 rounded-md bg-white/80"
//                             />
//                           </div>
...
//                             </label>
//                             <textarea
//                               className="w-full border border-[#CBBFAE] px-3 py-2 h-36 rounded-md bg-white/80"
//                               value={formData.TemplateData}
//                               onChange={(e) => onTemplateDataChange(e.target.value)}
//                             />
//                           </div>
//                         </>
//                       ) : formData.TemplateType === "sub" ? (
//                         <div>
//                           <label className="block text-sm font-semibold text-gray-700 mb-1">
//                             Topics + Sub-topics (one per line: Main - sub1, sub2)
//                           </label>
//                           <textarea
//                             className="w-full border border-[#CBBFAE] px-3 py-2 h-48 rounded-md bg-white/80"
//                             value={formData.TemplateData}
...
//                           <textarea
//                             className="w-full border border-[#CBBFAE] px-3 py-2 h-36 rounded-md bg-white/80"
//                             defaultValue={""}        // <-- prevents old data from showing
//                             onChange={(e) => onTemplateDataChange(e.target.value)}
...
//                         ) : (
//                           <>
//                             {/* REMOVE TEXTAREA — PDF SHOULD NOT BE EDITABLE */}

//                             <div className="mt-4 p-3 border bg-white rounded">
//                               <h4 className="text-md font-semibold text-gray-700">Live Preview</h4>

//                               <div className="text-sm text-gray-800 whitespace-pre-line mt-2">
//                                 {formData.TemplateType === "main" &&
//                                   (formData.TemplateData
//                                     ? formData.TemplateData.split(",").map(t => `✓ ${t.trim()}`).join("\n")
//                                     : pdfText || "PDF content will appear here...")}

//                                 {formData.TemplateType === "sub" &&
//                                   (formData.TemplateData || pdfText || "PDF content will appear here...")}
...
//                                         <img
//                                           src={formData.TemplateImage ? URL.createObjectURL(formData.TemplateImage) : ""}
//                                           alt="preview"
...
//                                     <div>{formData.TemplateData || pdfText || "Image description will appear here..."}</div>
//                                   </>
//                                 )}

//                                 {!formData.TemplateType && (pdfText || "PDF content will appear here...")}
//                               </div>
//                             </div>
//                           </>
//                         )}
...
>
  <FiUpload /> Skill Query
      ...
      {/* Right-side Skill Query panel (slide-over) */}
     {/* ===== BIG Skill Query POPUP ===== */}
{showSkillQuery && (
  <div
    className="fixed inset-0 z-50 flex items-center justify-center px-4"
    role="dialog"
  ...
  >
    ...
    <div
      ...
      onClick={() => setShowSkillQuery(false)}
    />
    ...
    >
     ...
     bg-gradient-to-r from-[#B87C4C] to-[#A66A3A]
     text-white font-semibold 
     sticky top-0 rounded-t-2xl border-b border-[#D8C7B2] shadow">
  
  <h3 className="text-xl tracking-wide">Skill Query</h3>
        <button
          onClick={() => setShowSkillQuery(false)}
          className="px-3 py-2 rounded-md hover:bg-gray-200 text-gray-800 font-bold text-lg"
        >
          ✕
        </button>
      </div>
      ...
      {/* Body */}
      <div className="p-6 overflow-auto h-[calc(85vh-80px)]">
        <SkillRequest userId={userId} />
      </div>
    </div>
  </div>
)}
