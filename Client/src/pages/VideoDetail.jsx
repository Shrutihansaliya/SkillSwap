...
export default function VideoDetail({ swapId, user, reload }) {
  ...
  if (loading)
    return <p className="text-center text-indigo-500 font-medium mt-4">Loading videos...</p>;

  if (videos.length === 0)
    return <p className="text-center text-gray-500 italic mt-4">No videos uploaded yet.</p>;
  ...
  return (
    <div className="bg-white/40 backdrop-blur-xl p-8 rounded-3xl shadow-lg">
      ...
      {/* ------------- YOUR VIDEOS ------------- */}
      <h3 className="text-xl font-semibold text-blue-700 mb-4">📌 Your Videos</h3>

      {myVideos.length === 0 ? (
        <p className="text-gray-500 text-sm mb-10">You haven't uploaded any videos.</p>
      ) : (
...
>
  ...
  <span className="text-[11px] font-medium">Delete</span>
      ...
      {/* ------------- PARTNER VIDEOS ------------- */}
      <h3 className="text-xl font-semibold text-purple-700 mb-4">🤝 Partner's Videos</h3>

      {partnerVideos.length === 0 ? (
        <p className="text-gray-500 text-sm">Your partner has not uploaded any videos.</p>
      ) : (
