 ...
 * - Same API endpoints & behavior preserved
 *
 * UPDATED: Approve now opens an "approve" modal to enter Category + Skill and Send.
 * On send:
 *  - POST /api/notifications { userId, message, link? }
 *  - PUT /api/skill-requests/:id/reply { status: "Approved", adminReply }
 */
...
export default function SkillRequestsFromUser() {
  ...
  // approve modal state
  const [openApproveFor, setOpenApproveFor] = useState(null);
  const [approveCategory, setApproveCategory] = useState("");
  const [approveSkillName, setApproveSkillName] = useState("");
  const approveCatRef = useRef(null);
  ...
  useEffect(() => {
    if (openApproveFor && approveCatRef.current) {
      setTimeout(() => approveCatRef.current.focus(), 80);
    }
  }, [openApproveFor]);
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE}/api/skill-requests`, { headers: adminHeaders, withCredentials: true });
      if (res.data && res.data.requests) setRequests(res.data.requests);
      else if (res.data && res.data.success && Array.isArray(res.data.requests)) setRequests(res.data.requests);
      else setRequests(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
  ...
  const openApproveModal = (reqId) => {
    const req = requests.find((r) => r._id === reqId || r.id === reqId);
    // prefill skill name if available
    setApproveSkillName(req?.SkillName || req?.skillName || "");
    setApproveCategory("");
    setOpenApproveFor(reqId);
    document.body.style.overflow = "hidden";
  };

  const closeApproveModal = () => {
    setOpenApproveFor(null);
    setApproveCategory("");
    setApproveSkillName("");
    document.body.style.overflow = "";
  };

  const updateRequestStatus = async (reqId, status, adminReply = null) => {
    if (!["Approved", "Rejected", "Pending"].includes(status)) {
      toast.error("Invalid status");
      return;
    }
    setActionLoading(true);
    try {
      const payload = { status, adminReply: adminReply ? adminReply.trim() : null };
      const res = await axios.put(`${API_BASE}/api/skill-requests/${reqId}/reply`, payload, {
  ...
  // Approve flow: create notification, then update status to Approved
 const approveWithCategory = async (reqId) => {
  if (!approveCategory.trim() || !approveSkillName.trim()) {
    toast.error("Please enter category & skill");
    return;
  }

  const adminReply = `Category: ${approveCategory}, Skill: ${approveSkillName}`;

  try {
    const res = await axios.put(
      `${API_BASE}/api/skill-requests/${reqId}/reply`,
      { status: "Approved", adminReply },
    ...
    if (res.data.success) {
      ...
      setRequests((prev) =>
        prev.map((req) =>
          req._id === reqId
            ? { ...req, Status: "Approved", AdminReply: adminReply }
            : req
        )
      );
    } else {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-3">
            {filtered.map((r) => (
              ...
              >
                ...
                <div className="w-full md:w-[240px] flex flex-col gap-2 items-stretch justify-center">
                  <button
                    onClick={() => openApproveModal(r._id || r.id)}
                    disabled={actionLoading}
                    className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-emerald-100 text-emerald-700 hover:bg-emerald-200 text-sm font-medium transition"
                  >
                    <FiCheckCircle />
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 justify-end mt-4">
                  ...
                  <button
                    onClick={() => {
                      if (!window.confirm("Are you sure you want to reject this request?")) return;
                      updateRequestStatus(openReplyFor, "Rejected", replyText);
                    }}
                    disabled={actionLoading}
                    className="px-3 py-1.5 rounded-lg border border-red-200 bg-red-50 text-red-600 hover:bg-red-100 text-xs"
                  >
      ...
      {/* Approve modal (category + skill + send) */}
      {openApproveFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-md" onClick={closeApproveModal} aria-hidden="true" />

          <div className="relative max-w-lg w-full mx-4 rounded-2xl bg-white border border-slate-100 shadow-2xl p-5 transform transition-all duration-200 ease-out animate-scale-in" role="dialog" aria-modal="true">
            <div className="flex items-start gap-4">
              <div className="w-2 h-10 rounded-full bg-gradient-to-b from-green-400 to-emerald-500" />
              <div className="flex-1">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="text-lg font-bold text-slate-800">Approve request</div>
                    <div className="text-sm text-slate-500 mt-1">Add category and confirm the skill to send a notification to the user.</div>
                  </div>
                  <button onClick={closeApproveModal} className="text-slate-400 hover:text-slate-600 rounded-full p-2">✕</button>
                </div>
                ...
                <div className="mt-4 grid grid-cols-1 gap-3">
                  ...
                  <input
                    ...
                    className="w-full p-3 rounded-lg border border-slate-100 text-sm outline-none focus:ring-2 focus:ring-emerald-100"
                  />

                  <label className="text-xs text-slate-600">Skill name</label>
                  <input
                    value={approveSkillName}
                    onChange={(e) => setApproveSkillName(e.target.value)}
                ...
                <div className="flex items-center justify-end gap-3 mt-4">
                  <button onClick={closeApproveModal} className="px-3 py-1.5 rounded-lg border text-xs">Cancel</button>

                  <button
                    onClick={() => {
                      if (!window.confirm("Send notification and approve this request?")) return;
                      approveWithCategory(openApproveFor);
                    }}
                    disabled={actionLoading}
                    className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs hover:opacity-95"
                  >
                    Send & Approve
                  </button>
                </div>
